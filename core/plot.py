...
def plotEvents(events, tracking, em, names, plot_players):
    
    colors = ['black','blue','brown','cornflowerblue','crimson','cyan','darkgreen','darkmagenta','darkred','forestgreen','gold','green','greenyellow','honeydew','hotpink','indianred','indigo','ivory','khaki','lavender','lavenderblush','lawngreen','lemonchiffon','lightblue','lightcoral','lightcyan','lightgoldenrodyellow','lightgreen','lightgray','lightpink','lightsalmon','lightseagreen','lightskyblue','lightslategray','lightsteelblue','lightyellow','lime','limegreen','linen','magenta','maroon','mediumaquamarine','mediumblue','mediumorchid','mediumpurple','mediumseagreen','mediumslateblue','mediumspringgreen','mediumturquoise','mediumvioletred','midnightblue','mintcream','mistyrose','moccasin','navajowhite','navy','oldlace','olive','olivedrab','orange','orangered','orchid','palegoldenrod','palegreen','paleturquoise','palevioletred','papayawhip','peachpuff','peru','pink','plum','powderblue','purple','red','rosybrown','royalblue','saddlebrown','salmon','sandybrown','seagreen','seashell','sienna','silver','skyblue','slateblue','slategray','snow','springgreen','steelblue','tan','teal','thistle','tomato','turquoise','violet','wheat','white','whitesmoke','yellow','yellowgreen']
    ...
    players = events['From'].unique()
    color_dict = dict(zip(players, colors[:len(players)]))

    for i, event in events.iterrows():

        x = event[em['SX']]*pitchLengthX
        y = (event[em['SY']]*pitchWidthY)-pitchWidthY
        
        if y < 0:
            y*=-1
        
        passCircle = plt.Circle((x, y), 2, color=color_dict[event[em['From']]])
        passCircle.set_alpha(.2)

        ax.add_patch(passCircle)

        dx = event[em['EX']]*pitchLengthX
        dy = (event[em['EY']]*pitchWidthY)-pitchWidthY
        ...
        passArrow = plt.Arrow(x, y, dx-x, dy-y,width=2,color=color_dict[event[em['From']]])
